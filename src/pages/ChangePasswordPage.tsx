...
// Resolve background image via URL for robust Vite handling (same as Login)
...
export default function ChangePasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!email || !current || !nextPw || !confirm) { setError('All fields are required.'); return }
  ...
  // Same right-panel quotes as Login for visual parity
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: 'radial-gradient(1200px 800px at 10% 10%, rgba(99,102,241,0.12), rgba(0,0,0,0)), linear-gradient(135deg,#0b1220 0%, #0f172a 40%, #111827 100%)' }}>
      ...
      <div className={`min-h-screen grid place-items-center px-4 sm:px-6 transition-opacity duration-500 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="relative w-full max-w-6xl rounded-[28px] overflow-hidden bg-white/5 border border-white/10 backdrop-blur-sm shadow-[0_40px_100px_rgba(0,0,0,0.45),inset_0_1px_0_rgba(255,255,255,0.05)] origin-center scale-[0.8]">
          <div className="grid grid-cols-1 lg:grid-cols-2">
            {/* Left: Change Password panel (form) */}
            <div className="p-8 md:p-12">
              ...
              <div className="max-w-md">
                ...
                <p className="mt-2 text-slate-300 text-sm">Keep your ELECTRIX account secure. Update your password below.</p>

                <form onSubmit={onSubmit} className="mt-6 space-y-4 text-[15px]">
                  <div>
                    ...
                    <div className="mt-2 relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-3 py-2.5 rounded-xl bg-white/90 text-slate-900 placeholder-slate-500 border border-white/20 focus:outline-none focus:ring-2 focus:ring-indigo-400/70 focus:border-indigo-400 transition"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs sm:text-sm">
                    <Link to="/login" className="text-white/70 hover:text-white">Back to login</Link>
                    <div className="text-white/50">Internal use only</div>
                  </div>

                  <div className="pt-2 text-center text-xs sm:text-sm text-white/60">© ELECTRIX 2025. All rights reserved.</div>
                </form>
              </div>
            </div>
